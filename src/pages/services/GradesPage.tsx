            ...
            {item ? (
              <div className="metric-grid metric-grid--rank">
                <div className="metric">
                  <p className="metric__label">算术平均</p>
                  <p className="metric__value">{item.arithmetic ?? "—"}</p>
                  <p className="muted text-sm">排名 {item.arithmetic_rank ?? "—"}</p>
                </div>
                <div className="metric">
                  <p className="metric__label">加权平均</p>
                  <p className="metric__value">{item.weighted ?? "—"}</p>
                  <p className="muted text-sm">排名 {item.weighted_rank ?? "—"}</p>
                </div>
                <div className="metric">
                  <p className="metric__label">GPA</p>
                  <p className="metric__value">{item.gpa ?? "—"}</p>
                  <p className="muted text-sm">排名 {item.gpa_rank ?? "—"}</p>
                </div>
              </div>
            ) : (
      ...
      >
        <div className="segmented">
          <button
            ...
            onClick={() => setSearchParams({ source: "school" })}
          >
            教务系统
          </button>
          <button
            ...
            onClick={() => setSearchParams({ source: "ca" })}
          >
            可信凭证
          </button>
        </div>
        {source === "ca" && caRank.data ? (
          <p className="muted text-sm">更新时间：{formatDateTime(caRank.data.updated_at)}</p>
        ) : null}
        {refreshCa.isError ? (
        ...
        {activeRank.isError ? (
          <PageError error={activeRank.error} onRetry={() => void activeRank.refetch()} />
        ) : null}
