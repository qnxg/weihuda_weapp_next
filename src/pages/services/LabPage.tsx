  ...
  return (
    <div className="page">
      <PageHeader
        ...
        description={
          grade.data
            ? `${grade.data.course_name} · 总评 ${grade.data.course_score ?? "待发布"}`
            : undefined
        }
        back
      />
      ...
      <Section title="实验安排">
        <QueryState query={schedule} loadingRows={4}>
          {(items) =>
            items.length ? (
              <div className="surface list">
                {items.map((item) => (
                  <article
                    className="record-item stack gap-8"
                    key={`${item.name}-${item.date_time}`}
                  >
      ...
      <Section title="实验成绩">
        <QueryState query={grade} loadingRows={3}>
          {(result) =>
            result.labs.length ? (
              <div className="surface list">
                {result.labs.map((lab) => (
