...
export default function LabPage() {
  ...
  const grade = useQuery({
    queryKey: ["lab", "grade", 2026, "autumn"],
    queryFn: () => api.lab.grade(2026, "autumn"),
  });
  ...
  return (
    <div className="page">
      <PageHeader
        title="大物实验"
        description={`${grade.data!.course_name} · 总评 ${grade.data!.course_score ?? "待发布"}`}
        back
      />
              ...
              <article className="record-item stack gap-8" key={`${item.name}-${item.date_time}`}>
                <div className="cluster spread gap-12">
                  ...
                  <span className="badge badge--brand">
                    第 {item.week} 周 {weekdays[item.day - 1]}
                  </span>
                </div>
                <p>
                ...
                <p className="muted text-sm">
                  {formatDateTime(item.date_time)} · {item.place} · 座位 {item.seat}
                </p>
                <div className="cluster gap-8">
                  {item.phone ? (
                    <a className="button button--ghost button--small" href={`tel:${item.phone}`}>
                      <Phone aria-hidden="true" />
                      联系教师
                    </a>
                  ) : null}
                  {item.email ? (
                    <a className="button button--ghost button--small" href={`mailto:${item.email}`}>
                      <Mail aria-hidden="true" />
                      发送邮件
                    </a>
                  ) : null}
                  {!item.phone && !item.email ? (
                    <span className="muted text-sm">暂无联系方式</span>
                  ) : null}
                </div>
              </article>
            ))}
            ...
            {grade.data!.labs.map((lab) => (
              <article className="record-item stack gap-8" key={lab.lab_name}>
                ...
                {lab.details.map((detail) => (
                  <div className="cluster spread text-sm" key={detail.name}>
                    <span>{detail.name}</span>
                    <strong>{detail.score ?? "待发布"}</strong>
                  </div>
                ))}
              </article>
            ))}
      ...
      <Section title="绑定实验平台" description="绑定信息只用于同步实验安排与成绩。">
        <form className="form surface surface--padded" onSubmit={submitBind}>
          <div className="field">
            <label htmlFor="lab-password">实验平台密码</label>
            <input
              id="lab-password"
              name="password"
              type="password"
