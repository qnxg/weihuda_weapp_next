...
export default function NetworkPage() {
  ...
  return (
    <div className="page">
      ...
      {network.is_locked ? (
        <StatusMessage tone="danger"><LockKeyhole aria-hidden="true" />账号已锁定，请先处理欠费。</StatusMessage>
      ) : network.overdue_payment > 0 ? (
        <StatusMessage tone="warning">待缴费用 {formatCurrency(network.overdue_payment)}</StatusMessage>
      ) : (
        <StatusMessage tone="success"><Wifi aria-hidden="true" />校园网状态正常</StatusMessage>
      )}
      ...
      <Section title="本月用量">
        <div className="surface surface--padded stack gap-12">
          ...
          <div className="metric-grid">
            <div className="metric"><p className="metric__label">套餐流量</p><p className="metric__value">{network.base_amount}</p></div>
            <div className="metric"><p className="metric__label">基础用量</p><p className="metric__value">{formatNumber(network.base_usage)} GB</p></div>
            <div className="metric"><p className="metric__label">扩展用量</p><p className="metric__value">{formatNumber(network.extend_usage)} GB</p></div>
            <div className="metric"><p className="metric__label">上传 / 下载</p><p className="text-sm"><strong>{network.upload}</strong> / <strong>{network.download}</strong></p></div>
          </div>
        </div>
      </Section>

      <Section title="应用明细" description={`合计 ${detail.data!.total} · 上传 ${detail.data!.upload} · 下载 ${detail.data!.download}`}>
          ...
          <div className="surface list">
            {detail.data!.items.map((item) => (
              <article className="record-item stack gap-8" key={item.app}>
                ...
                <progress className="progress" value={Math.min(item.percentage, 1)} max={1}>{formatNumber(item.percentage * 100)}%</progress>
                <p className="muted text-sm">下载 {item.download} · 上传 {item.upload} · 占比 {formatNumber(item.percentage * 100)}%</p>
              </article>
            ))}
          </div>
          ...
          <div className="surface list">
            {orders.data!.map((order) => (
              <article className="record-item" key={`${order.year}-${order.month}`}>
                <div className="cluster spread gap-12"><h3>{order.year} 年 {order.month} 月</h3><strong>{formatCurrency(order.amount)}</strong></div>
                <p className="muted text-sm">下载 {order.download} · 上传 {order.upload} · 超额 {order.over}</p>
