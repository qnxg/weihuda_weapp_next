...
export default function AboutPage() {
  ...
  if (about.isPending) return <div className="page"><PageSkeleton rows={4} /></div>;
  if (about.isError) {
    return (
      <div className="page">
  ...
  return (
    <div className="page">
      ...
      <section className="profile-heading">
        <div className="login-brand__mark" aria-hidden="true">微</div>
        <div>
          <h2>微生活</h2>
      ...
      <Section title="项目链接">
        <div className="surface list">
          <a className="menu-link" href={about.data.home} target="_blank" rel="noreferrer">
            <ExternalLink aria-hidden="true" /><span>项目主页</span><span className="menu-link__meta muted text-sm">{about.data.home}</span>
          </a>
          <a className="menu-link" href={about.data.join} target="_blank" rel="noreferrer">
            <ExternalLink aria-hidden="true" /><span>加入我们</span><span className="menu-link__meta muted text-sm">{about.data.join}</span>
          </a>
        </div>
      </Section>
      ...
      <Section title="服务状态">
        ...
        {health.isError ? (
          ...
          <StatusMessage tone="success"><Radio aria-hidden="true" />服务连接正常</StatusMessage>
        )}
      </Section>
    </div>
  );
