    ...
    <div className="page">
        ...
        <form className="form surface surface--padded" onSubmit={submit}>
          ...
          <div className="field">
            ...
            <textarea
              ...
              required
              minLength={8}
              placeholder="请描述发生了什么、出现问题的页面和期望结果…"
            />
          </div>
          {isAuthenticated ? (
            <div className="field">
          ...
          {submitFeedback.isError ? (
            <StatusMessage tone="danger">{submitFeedback.error.message}</StatusMessage>
          ) : null}
          <button
      ...
      {isAuthenticated ? (
        <Section title="我的反馈">
          ...
          {history.isError ? (
            <PageError error={history.error} onRetry={() => void history.refetch()} />
          ) : null}
              ...
              {history.data.items.map((item) => (
                <article className="record-item stack gap-8" key={item.id}>
                  <div className="cluster spread gap-12">
                    <span
                      className={`badge ${item.status === "resolved" ? "badge--success" : "badge--warning"}`}
                    >
                      {item.status}
                    </span>
                  ...
                  <p className="muted text-sm">
                    联系方式：{item.contact || "未提供"} · 更新于 {formatDateTime(item.updated_at)}
                  </p>
                  {item.img ? (
                    <AuthenticatedImage id={item.img} alt={`反馈 ${item.id} 的附件`} />
                  ) : null}
          ...
          {history.data && history.data.total > history.data.items.length ? (
            <button
              ...
              onClick={() => setHistorySize((size) => size + 20)}
            >
        ...
        <EmptyState
          title="匿名反馈"
          description="你当前未登录，反馈提交成功后不会显示处理记录。"
          icon={ImageIcon}
        />
      )}
    </div>
