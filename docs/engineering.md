...
首页倒计时由 `CountdownOverview` 统一展示：`src/utils/holiday.ts` 维护已公布的法定假期日期，`src/utils/format.ts` 根据学期开始日期与教学周数计算学期边界。两者都以 UTC 日历序号计算本地自然日差值，复用首页时钟，不创建独立定时器。

首页通过不带 `xn/xq` 的 `GET /semester` 获取当前学期，再以响应中的学年和学期参数请求课表与成绩；考试、通知和首页设置等无依赖请求继续并行发起。
...
- 单元测试：首页日期标题、周次与学期边界计算、连续课程合并、课程时间状态、假期自然日差值、响应解析与格式化。
- 组件测试：今日/明日切换、默认明日、课程状态卡、假期与学期倒计时状态、骨架、错误重试、空状态、鉴权门禁和表单校验。
